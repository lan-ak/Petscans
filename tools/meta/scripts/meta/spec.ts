...
/** One card of a carousel. `image` is a path (uploaded at launch); `imageHash` is already uploaded. */
...
export interface CreativeSpec {
  ...
  primaryText: string;
  /** Optional when `cards` is used — a carousel has no single headline. */
  headline?: string;
  description?: string;
  cta?: string;
  /** Exactly one of `media` (single image/video) or `cards` (2–10 carousel cards). */
  media?: MediaSpec;
  cards?: CardSpec[];
}
...
const CREATIVE_KEYS = new Set([
  'name', 'pageId', 'instagramActorId', 'primaryText', 'headline', 'description', 'cta', 'media', 'cards',
]);
const CARD_KEYS = new Set(['image', 'imageHash', 'headline', 'description', 'link']);
...
export function parseSpec(raw: unknown, baseDir: string): LaunchSpec {
  ...
  // The 5-ad-set ceiling is a SKAdNetwork rule (subcode 2490238), so it only binds an
  // app-promotion campaign. A traffic campaign is not SKAN and can hold more.
  const trafficCampaign = campaign.objective === 'OUTCOME_TRAFFIC';
  if (!trafficCampaign && root.adsets.length > 5) {
    throw new ValidationError(
      `spec.adsets has ${root.adsets.length} ad sets. A SKAdNetwork app-promotion campaign allows at most 5 (subcode 2490238).`,
      'For more than 5 one-creative-each ad sets, use a traffic campaign: set spec.campaign.objective to "OUTCOME_TRAFFIC".',
    );
  }
...
function parseAd(raw: unknown, path: string, baseDir: string): AdSpec {
  ...
  const hasMedia = c.media !== undefined;
  const hasCards = c.cards !== undefined;
  if (hasMedia && hasCards) {
    throw new ValidationError(
      `${path}.creative has both "media" and "cards".`,
      'A creative is either a single image/video ("media") or a carousel ("cards"), not both.',
    );
  }
  if (!hasMedia && !hasCards) {
    throw new ValidationError(
      `${path}.creative needs "media" (single image/video) or "cards" (2–10 carousel cards).`,
    );
  }

  const adName = requireString(a.name, `${path}.name`);
  const creative: CreativeSpec = {
    name: (c.name as string) ?? `${adName} — creative`,
  ...
  if (hasCards) {
    creative.cards = parseCards(c.cards, `${path}.creative.cards`, baseDir);
    // A carousel has no single headline (each card carries its own). Keep it if given.
    if (c.headline !== undefined) creative.headline = requireString(c.headline, `${path}.creative.headline`);
  } else {
    // Single image/video. Headline is required here — it's the ad's one headline.
    creative.headline = requireString(c.headline, `${path}.creative.headline`);
    creative.media = parseMedia(c.media, `${path}.creative.media`, baseDir);
  }

  return { name: adName, creative };
}

function parseMedia(raw: unknown, path: string, baseDir: string): MediaSpec {
  if (typeof raw !== 'object' || raw === null) {
    throw new ValidationError(`${path} is required — an ad needs an image or a video.`);
  }
  const m = raw as Record<string, unknown>;
  rejectUnknown(m, MEDIA_KEYS, path);

  if (m.type !== 'image' && m.type !== 'video') {
    throw new ValidationError(`${path}.type must be "image" or "video".`);
  }
  if (m.type === 'image' && !m.path && !m.hash) {
    throw new ValidationError(`${path} needs a "path" (to upload) or a "hash" (already uploaded).`);
  }
  if (m.type === 'video' && !m.path && !m.id) {
    throw new ValidationError(`${path} needs a "path" (to upload) or an "id" (already uploaded).`);
  }
  ...
  if (m.path) media.path = resolveMedia(m.path as string, baseDir, `${path}.path`);
  if (m.thumbnailPath) media.thumbnailPath = resolveMedia(m.thumbnailPath as string, baseDir, `${path}.thumbnailPath`);
...
function parseCards(raw: unknown, path: string, baseDir: string): CardSpec[] {
  if (!Array.isArray(raw)) throw new ValidationError(`${path} must be an array of cards.`);
  // Meta carousels take 2–10 cards.
  if (raw.length < 2 || raw.length > 10) {
    throw new ValidationError(`${path} must have between 2 and 10 cards. Got ${raw.length}.`);
  }
  return raw.map((rawCard, k) => {
    const cp = `${path}[${k}]`;
    if (typeof rawCard !== 'object' || rawCard === null) throw new ValidationError(`${cp} must be an object.`);
    const card = rawCard as Record<string, unknown>;
    rejectUnknown(card, CARD_KEYS, cp);
    if (!card.image && !card.imageHash) {
      throw new ValidationError(`${cp} needs an "image" (path to upload) or an "imageHash" (already uploaded).`);
    }
    const out: CardSpec = {};
    if (card.image) out.image = resolveMedia(card.image as string, baseDir, `${cp}.image`);
    if (card.imageHash) out.imageHash = card.imageHash as string;
    if (card.headline !== undefined) out.headline = requireString(card.headline, `${cp}.headline`);
    if (card.description !== undefined) out.description = requireString(card.description, `${cp}.description`);
    if (card.link !== undefined) out.link = requireString(card.link, `${cp}.link`);
    return out;
  });
}
