...
// This CLI builds two campaign shapes:
//   OUTCOME_APP_PROMOTION — iOS installs, SKAdNetwork, promoted_object = the app.
//   OUTCOME_TRAFFIC       — link clicks to a URL (the App Store listing, for us). No SKAN,
//                           no promoted_object; the creative carries the destination link.
const SUPPORTED_OBJECTIVES = new Set(['OUTCOME_APP_PROMOTION', 'OUTCOME_TRAFFIC']);
...
export function buildCampaign(input: CampaignInput, config: Config): Params {
  ...
  const objective = input.objective ?? 'OUTCOME_APP_PROMOTION';
  if (!SUPPORTED_OBJECTIVES.has(objective)) {
    throw new ValidationError(
      `Unsupported objective "${objective}".`,
      'Supported: OUTCOME_APP_PROMOTION (iOS installs, SKAN) and OUTCOME_TRAFFIC (link clicks to a URL).',
    );
  }

  // Only app-promotion campaigns are SKAdNetwork and carry a promoted_object. A traffic
  // campaign sends clicks to a URL, so the SKAN flag never applies to it.
  const appPromotion = objective === 'OUTCOME_APP_PROMOTION';
  const skan = appPromotion && input.skadnetwork !== false;
  ...
  if (hasCampaignBudget) {
    ...
    // No campaign budget means ad-set budgets (ABO). Meta now requires an explicit choice on
    // whether those ad sets share 20% of budget to optimize overall (subcode 4834011). We say
    // false: each ad set keeps its own fixed budget — exactly what a clean per-ad-set creative
    // test needs, since sharing would let Meta starve a "losing" creative and bias the read.
