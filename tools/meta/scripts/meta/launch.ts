...
import { buildCreative, CardInput } from './creatives';
...
export async function launch(ctx: Ctx): Promise<void> {
  ...
  try {
    // Traffic campaigns get link creatives (CTA carries the destination link, not the app).
    const linkOnly = spec.campaign.objective === 'OUTCOME_TRAFFIC';
    ...
    for (const [i, adset] of spec.adsets.entries()) {
      ...
      for (const [j, ad] of adset.ads.entries()) {
        await ensureAd(ctx, ad, `${i}.${j}`, adsetId, ledger, adoptExisting, linkOnly);
      }
    }
      ...
      buildCreative(
        {
          ...
          cards: ad.creative.cards?.map(
            (card): CardInput => ({
              imageHash: card.imageHash ?? 'validate',
          ...
          linkOnly: spec.campaign.objective === 'OUTCOME_TRAFFIC',
        },
        ctx.config,
      );
...
async function ensureAd(
  ...
  const creativeId = await ensureCreative(ctx, ad, key, ledger, linkOnly);
...
async function ensureCreative(ctx: Ctx, ad: AdSpec, key: string, ledger: Ledger, linkOnly: boolean): Promise<string> {
  ...
  if (ad.creative.cards) {
    // One card image per child attachment. Each upload is its own ledger step so a resume
    // never re-uploads a card that already landed.
    cards = [];
    for (const [c, card] of ad.creative.cards.entries()) {
      const hash = card.imageHash ?? (await uploadImageStep(ctx, card.image!, `image.${key}.c${c}`, ledger));
      cards.push({ imageHash: hash, headline: card.headline, description: card.description, link: card.link });
    }
  } else if (media?.type === 'image' && !imageHash && media.path) {
