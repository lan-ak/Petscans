...
export class MetaClient {
  ...
  private async activationBudget(objectId: string): Promise<number> {
    // An ad node has NO `daily_budget` field — requesting it makes Graph reject the whole
    // call (code 100), not return null. Tolerate that and fall through to the parent-campaign
    // lookup below, exactly as for an ad set under CBO. Without this, `ad resume` is impossible.
    let own = 0;
    try {
      own = Number(
