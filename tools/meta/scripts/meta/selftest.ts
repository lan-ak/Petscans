...
const CASES: Case[] = [
  ...
  {
    name: 'creative: carousel builds child_attachments',
    run: () =>
      buildCreative(
        {
          name: 'C',
          primaryText: 'body',
          cards: [
            { imageHash: 'h1', headline: 'Card 1', description: 'd1' },
            { imageHash: 'h2', headline: 'Card 2' },
          ],
        },
        FIXTURE,
      ),
    expect: (r) => {
      const link = r.object_story_spec?.link_data;
      const kids = link?.child_attachments;
      if (!Array.isArray(kids) || kids.length !== 2) return 'child_attachments should have 2 cards';
      if (link?.image_hash) return 'a carousel must NOT set a single top-level image_hash';
      if (kids[0].image_hash !== 'h1' || kids[1].image_hash !== 'h2') return 'card image_hash not carried through';
      if (kids[0].name !== 'Card 1') return 'card name (headline) not carried through';
      if (kids[0].call_to_action?.type !== 'INSTALL_MOBILE_APP') return 'each card CTA should be INSTALL_MOBILE_APP';
      if (!kids[0].call_to_action?.value?.application) return 'each card CTA must reference the app (application id)';
      if (link?.message !== 'body') return 'the primary text should be link_data.message';
      return undefined;
    },
  },
  {
    name: 'creative: rejects a carousel that also has a single image',
    run: () =>
      buildCreative(
        { name: 'C', primaryText: 'b', imageHash: 'x', cards: [{ imageHash: 'h1' }, { imageHash: 'h2' }] },
        FIXTURE,
      ),
    throws: 'not a single image',
  },
  {
    name: 'campaign: traffic objective — no SKAN, no promoted_object',
    run: () => buildCampaign({ name: 'T', objective: 'OUTCOME_TRAFFIC', dailyBudgetCents: 3000 }, FIXTURE),
    expect: (r) =>
      r.objective !== 'OUTCOME_TRAFFIC'
        ? `objective should be OUTCOME_TRAFFIC, got ${r.objective}`
        : r.is_skadnetwork_attribution
          ? 'a traffic campaign must NOT be SKAdNetwork'
          : r.promoted_object
            ? 'a traffic campaign must NOT carry a promoted_object'
            : undefined,
  },
  {
    name: 'campaign: rejects an unsupported objective',
    run: () => buildCampaign({ name: 'X', objective: 'OUTCOME_SALES' }, FIXTURE),
    throws: 'Unsupported objective',
  },
  {
    name: 'campaign: ABO (no campaign budget) declares budget-sharing off',
    run: () => buildCampaign({ name: 'ABO', objective: 'OUTCOME_TRAFFIC' }, FIXTURE),
    expect: (r) =>
      r.is_adset_budget_sharing_enabled !== false
        ? 'a campaign without campaign-level budget must set is_adset_budget_sharing_enabled=false (subcode 4834011)'
        : r.daily_budget
          ? 'an ABO campaign must not carry a campaign-level budget'
          : undefined,
  },
  {
    name: 'adset: traffic (LINK_CLICKS) — WEBSITE destination, no SKAN, iOS targeting',
    run: () =>
      buildAdSet(
        { name: 'AS', campaignId: '1', dailyBudgetCents: 1000, optimizationGoal: 'LINK_CLICKS' },
        FIXTURE,
      ),
    expect: (r) =>
      r.optimization_goal !== 'LINK_CLICKS'
        ? 'optimization_goal should be LINK_CLICKS'
        : r.destination_type !== 'WEBSITE'
          ? 'destination_type must be WEBSITE for a traffic ad set'
          : r.promoted_object
            ? 'a traffic ad set must NOT carry a promoted_object'
            : r.is_skadnetwork_attribution
              ? 'a traffic ad set must NOT be SKAdNetwork'
              : !(r.targeting as any)?.user_os?.includes('iOS')
                ? 'App Store traffic should target iOS users'
                : undefined,
  },
  {
    name: 'creative: link-only carries the link but not the app',
    run: () =>
      buildCreative(
        { name: 'C', primaryText: 'body', headline: 'head', imageHash: 'abc123', linkOnly: true },
        FIXTURE,
      ),
    expect: (r) => {
      const cta = r.object_story_spec?.link_data?.call_to_action;
      if (cta?.value?.application) return 'a link-only CTA must NOT reference the app';
      if (!cta?.value?.link) return 'the CTA must carry the destination link';
      return undefined;
    },
  },
  {
